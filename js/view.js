...
const View = {
  ...
  init() {
    ...
    if (this.els.bgm) this.els.bgm.volume = 0.3;

    // Start background music on first user interaction
    const startBGM = () => {
      if (this.els.bgm && this.els.bgm.paused) {
        this.els.bgm.play().catch(() => {});
  ...
  renderRepos(repos, statusText, model) {
    ...
    repos.forEach((r, i) => {
      ...
      a.innerHTML = `
        ...
        <p>${r.description || (r.name.toLowerCase() === "portfolio" ? "Need to know about me, click and find out" : "No description yet, but the code speaks for itself.")}</p>
        <div class="meta">
          <span></span>
  ...
  startClock() {
    setInterval(() => {
      this.els.clock.textContent =
        new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) + " · ABD";
    }, 1000);
  },
  ...
  startCursor() {
    ...
    addEventListener("mousemove", e => {
      // Divide by body zoom factor so the cursor doesn't drift when CSS zoom is applied
